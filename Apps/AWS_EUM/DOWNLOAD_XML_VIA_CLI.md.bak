...
2. Download the XML template (use the RAW GitHub link):
   ```bash
   wget -O /boot/config/plugins/dockerMan/templates-user/my-AWS_EUM_v3.xml \
        https://raw.githubusercontent.com/N85UK/UNRAID_Apps/main/Apps/AWS_EUM_v3/my-aws-eum-v3.xml
   chmod 644 /boot/config/plugins/dockerMan/templates-user/my-AWS_EUM_v3.xml
   ```
...
4. Add the container from the Unraid GUI:
   - Go to **Docker** tab
   - Click **Add Container**
   - Select **AWS_EUM_v3** from the **User templates** dropdown
